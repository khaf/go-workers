...
func EnqueueSpec(c gospec.Context) {
	...
	c.Specify("EnqueueIn", func() {
		...
		c.Specify("has added a job in the scheduled queue", func() {
			_, err := EnqueueIn("enqueuein1", "Compare", 10*time.Second, map[string]interface{}{"foo": "bar"})
		...
		c.Specify("has the correct 'queue'", func() {
			_, err := EnqueueIn("enqueuein2", "Compare", 10*time.Second, map[string]interface{}{"foo": "bar"})
			c.Expect(err, Equals, nil)
